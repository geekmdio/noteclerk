...
func (n *Server) CreateNote(ctx context.Context, nr *ehrpb.CreateNoteRequest) (*ehrpb.CreateNoteResponse, error) {
	...
	if nr.Note.GetId() > 0 {
		return nil, errors.New(ErrMapStr[ErrNoteClerkServerCreateNoteRejectsNoteDueToId])
	}

	id, _, err := n.db.AddNote(noteToAdd)
	if err != nil {
		err := errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerCreateNoteFailsAddNoteToDb])
		log.Warn(err)
...
func (n *Server) DeleteNote(ctx context.Context, dnr *ehrpb.DeleteNoteRequest) (*ehrpb.DeleteNoteResponse, error) {
	...
	if err != nil {
		err := errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerDeleteNoteFailsDeleteNoteFromDb])
		log.Warn(err)
...
func (n *Server) RetrieveNote(ctx context.Context, rnr *ehrpb.RetrieveNoteRequest) (*ehrpb.RetrieveNoteResponse, error) {
	...
	if err != nil {
		err := errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerRetrieveNoteFailsToGetNoteFromDb])
		log.Warn(err)
...
func (n *Server) SearchNotes(ctx context.Context, fnr *ehrpb.SearchNotesRequest) (*ehrpb.SearchNotesResponse, error) {
	...
	if err != nil {
		err := errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerSearchNotesFailsToFindNotesInDb])
		log.Warn(err)
		res.Status.HttpCode = ehrpb.StatusCodes_NOT_FOUND
...
func (n *Server) UpdateNote(ctx context.Context, unr *ehrpb.UpdateNoteRequest) (*ehrpb.UpdateNoteResponse, error) {
	...
	if unr.Id != unr.Note.Id {
		newErr := errors.New(ErrMapStr[ErrNoteClerkServerUpdateNoteFailsDueToIdMismatch])
		log.Warn(newErr)
	...
	if err != nil {
		newErr := errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerUpdateNoteFailsToUpdateNoteInDb])
		log.Warn(newErr)
...
func (n *Server) Initialize(config *Config, db RDBMSAccessor) error {
	...
	if err != nil {
		return errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerInitializeFailsDbInitialization])
	}
	...
	if err != nil {
		return errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerInitializeFailsCreateListener])
	}
	...
	if err = n.server.Serve(lis); err != nil {
		return errors.WithMessage(err, ErrMapStr[ErrNoteClerkServerInitializeFailsInitializingRpcServer])
	}
...
func (n *Server) constructor(config *Config, db RDBMSAccessor) error {
	if db == nil {
		return errors.New(ErrMapStr[ErrNoteClerkServerConstructorFailsDueToNilDb])
	}
	if config == nil {
		return errors.New(ErrMapStr[ErrNoteClerkServerConstructorFailsDueToNilConfig])
	}
